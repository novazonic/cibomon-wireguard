#!/bin/bash
#
# https://github.com/novazonic/cibomon-wireguard
#
# Copyright (c) 2025 Cibomon. Released under the MIT License.
...
PresharedKey = $psk
AllowedIPs = 100.101.102.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
	# Create client configuration
	cat << EOF > "$script_dir"/"$client".conf
[Interface]
Address = 100.101.102.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key
...
[Interface]
Address = 100.101.102.1/24$([[ -n "$ip6" ]] && echo ", fddd:2c4:2c4:2c4::1/64")
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd --zone=trusted --add-source=100.101.102.0/24
		firewall-cmd --permanent --add-port="$port"/udp
		firewall-cmd --permanent --zone=trusted --add-source=100.101.102.0/24
		# Set NAT for the VPN subnet
		firewall-cmd --direct --add-rule ipv4 nat POSTROUTING 0 -s 100.101.102.0/24 ! -d 100.101.102.0/24 -j SNAT --to "$ip"
		firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 100.101.102.0/24 ! -d 100.101.102.0/24 -j SNAT --to "$ip"
		if [[ -n "$ip6" ]]; then
...
[Service]
Type=oneshot
ExecStart=$iptables_path -w 5 -t nat -A POSTROUTING -s 100.101.102.0/24 ! -d 100.101.102.0/24 -j SNAT --to $ip
ExecStart=$iptables_path -w 5 -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -s 100.101.102.0/24 -j ACCEPT
ExecStart=$iptables_path -w 5 -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -w 5 -t nat -D POSTROUTING -s 100.101.102.0/24 ! -d 100.101.102.0/24 -j SNAT --to $ip
ExecStop=$iptables_path -w 5 -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -w 5 -D FORWARD -s 100.101.102.0/24 -j ACCEPT
	...
	case "$option" in
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 100.101.102.0/24 '"'"'!'"'"' -d 100.101.102.0/24' | grep -oE '[^ ]+$')
					# Using both permanent and not permanent rules to avoid a firewalld reload.
					firewall-cmd --remove-port="$port"/udp
					firewall-cmd --zone=trusted --remove-source=100.101.102.0/24
					firewall-cmd --permanent --remove-port="$port"/udp
					firewall-cmd --permanent --zone=trusted --remove-source=100.101.102.0/24
					firewall-cmd --direct --remove-rule ipv4 nat POSTROUTING 0 -s 100.101.102.0/24 ! -d 100.101.102.0/24 -j SNAT --to "$ip"
					firewall-cmd --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 100.101.102.0/24 ! -d 100.101.102.0/24 -j SNAT --to "$ip"
